...
function moveHead(pos, dir) {
  ...
  if (dir === "L") return Math.max(0, pos - 1);
  return pos; // S = Stay
}
...
const PRESETS = {
  copier: {
    label: "Binary Copier (2-tape, O(n))",
  ...
  addition: {
    label: "Unary Addition (3-tape, O(n))",
    description: "Adds two unary numbers using 3 tapes in O(n). Encodes as 111+11 for 3+2=5.",
    numTapes: 3,
    input: "111+11",
    acceptStates: ["qAccept"],
    rejectStates: [],
    initialState: "q0",
    rules: [
      { from: "q0", reads: ["1", "_", "_"], to: "q0", writes: ["1", "1", "_"], moves: ["R", "R", "S"] },
      { from: "q0", reads: ["+", "_", "_"], to: "q1", writes: ["+", "_", "_"], moves: ["R", "S", "S"] },
      { from: "q1", reads: ["1", "_", "_"], to: "q1", writes: ["1", "1", "_"], moves: ["R", "R", "S"] },
      { from: "q1", reads: ["_", "_", "_"], to: "qAccept", writes: ["_", "_", "_"], moves: ["S", "S", "S"] },
    ],
  },
  singleAnBn: {
...
export default function App() {
  ...
  function loadPreset(key) {
    ...
    setNumTapes(p.numTapes);
    setInputStr(p.input);
    setRules(p.rules);
    setAcceptStates(p.acceptStates);
    setRejectStates(p.rejectStates || []);
    setConfig(null);
    setSteps(0);
    setHistory([]);
    setIsPlaying(false);
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: "#334155", fontFamily: "monospace", letterSpacing: "0.08em" }}>
              k-tape simulator · state diagram · efficiency analysis
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          ...
          <div style={{
            ...
            {[1, 2, 3].map(n => (
              <button
                key={n}
                onClick={() => { setNumTapes(n); setConfig(null); setSteps(0); setHistory([]); }}
                style={{
                  padding: "6px 12px",
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {config ? (
                  ...
                  <div style={{
                    ...
                    fontFamily: "monospace", fontSize: 13, padding: 60,
                  }}>
